...
class HC_Alsing():
    ...
    def __init__(self, pop_size, num_children, ref_img, board='white',
                 ...
                 lim_alpha_min=30, lim_alpha_max=60, lim_move_pos_min=3, lim_move_pos_max=20):
    ...
    def _mutate_figure(self, pol, col):
        ...
        for i in mutated_pos_med:
            k, xy = divmod(i, 2)
            vert = copy(pol[k])
            vert[xy] = self._crop(vert[xy] + random.randint(-self.lim_move_pos_max, self.lim_move_pos_max), xy)
            pol = pol[:k] + [vert] + pol[k + 1:]
            
        for i in mutated_pos_low:
            k, xy = divmod(i, 2)
            vert = copy(pol[k])
            vert[xy] = self._crop(vert[xy] + random.randint(-self.lim_move_pos_min, self.lim_move_pos_min), xy)
    ...
    def save(self, name):
        name = name + '_hcAls_%i_%i_%i_%i_%i' % (self.pop_size, self.num_children, self.max_num_figs, 
...
class HC_Alsing_mod():
    ...
    def save(self, name):
        name = name + '_hcAlsmod_%i_%i_%i_%i_%i' % (self.pop_size, self.num_children, self.max_num_figs, 
                                                   self.max_num_edges, self.iterations_done)
        with open(name + '.pkl', 'w') as f:
            pickle.dump(self.log, f)
