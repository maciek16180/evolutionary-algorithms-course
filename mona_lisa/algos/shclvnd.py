...
class SHCLVND():
    ...
    def __init__(self, pop_size, learning_rate, std_init, std_decay, ref_img,
                 F='default', n_best=2, num_edges=3, num_figs=50, mode='polygon_simple'):
    ...
    def save(self, name):
        name = name + '_shclvnd_%i_%.3f_%.3f_%.3f_%i_%i_%i' % (self.pop_size, self.learning_rate, 
...
class SHCLVND_perm():
    ...
    def save(self, name):
        name = name + '_shclvndp_%i_%.3f_%.3f_%.3f_%i_%i_%i' % (self.pop_size, self.learning_rate, 
                                                                self.std_init, self.std_decay,
                                                                self.num_figs, self.num_edges, self.iterations_done)
        with open(name + '.pkl', 'w') as f:
            pickle.dump(self.log, f)
